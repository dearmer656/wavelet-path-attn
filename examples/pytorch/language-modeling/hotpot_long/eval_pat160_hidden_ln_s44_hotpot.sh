#!/bin/bash
#SBATCH --job-name=P160_hln_s44_hq
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/logs/%j_pat160_hidden_ln_s44_hotpot.txt
...
# PAT-160: HotpotQA-Long Uniform eval for QWAB-Hidden-LN s44
...
CKPT="${BASE}/runs/mix_medium_owt_hidden_ln_10ep_s44/checkpoint-15000"
CFG="${BASE}/runs/mix_medium_owt_hidden_ln_10ep_s44/supply_model.cfg"
JSONL="${BASE}/hotpot_long/data/hotpot_long_dev_uniform.jsonl"
cd "${BASE}"

for L in 512 2048 4096; do
  OUTPUT="${BASE}/hotpot_long/results_uniform/mix_medium_owt_hidden_ln_10ep_s44_ckpt15000/L${L}"
  mkdir -p "${OUTPUT}"
  echo "=== Hidden-LN s44 L${L} ==="
  ...
  python -m torch.distributed.run --nproc_per_node=4 --master_port=${MASTER_PORT} ./run_clm.py \
    ...
    --seed 44 --load_best_model_at_end False \
  ...
  python3 -c "import json; d=json.load(open('${OUTPUT}/eval_results.json')); print(f'Hidden-LN s44 L${L}: F1={d[\"eval_f1\"]:.4f} EM={d[\"eval_em\"]:.4f}')"
done

echo "=== Done: PAT-160 Hidden-LN s44 HotpotQA L512/L2048/L4096 ==="
