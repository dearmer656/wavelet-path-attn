#!/bin/bash
# Evaluate PA RULER-512 finetuned checkpoint on official RULER sets (L2048, L4096), full-size.

#SBATCH --job-name=ev_ruler_pa_of
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/logs/%j_ev_ruler_pa_official_elm55.txt
...
CKPT="${BASE}/runs/ruler_ft_len_generalize_20260525/path_pa_s42_ruler512_ft1k/checkpoint-1000"
CFG="${BASE}/runs/mix_medium_owt_PA_10ep/supply_model.cfg"
...
run_one() {
  ...
  local data_file="$2"
  local bs="$3"
  local out_dir="${BASE}/hotpot_long/results_ruler_ft_official/path_pa_s42_ruler512_ft1k/${tag}"
  mkdir -p "${out_dir}"
...
echo "Done. Results saved under ${BASE}/hotpot_long/results_ruler_ft_official/path_pa_s42_ruler512_ft1k/"
