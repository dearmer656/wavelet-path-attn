#!/bin/bash
#SBATCH --job-name=pat200_attn_effects
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/hotpot_long/logs/%j_pat200_attn_effects.txt
...
_slack() {
    python3 /project/nlp-work5/hongyu-s/gate1/scripts/notify_slack.py \
        ...
        --issue "PAT-200" --gpu "1xA6000" \
        --summary "PAT-200 Phase B attention-effect dump (QWAB on vs off): L4096 NCASE=${NCASE} (${OUTSUB})" 2>/dev/null || true
}
trap '_slack $?' EXIT
set -euxo pipefail
...
python hotpot_long/dump_scale_attention_effects.py \
    --checkpoint ${CKPT} \
    --jsonl hotpot_long/data/hotpot_long_dev_uniform.jsonl \
    --out_dir hotpot_long/analysis_outputs/qwab_scale_query_nmf/${OUTSUB} \
    --seq_len 4096 --n_case ${NCASE} --query_budget ${BUDGET} --min_pos 16 --seed 42

echo "=== Done: PAT-200 Phase B attention-effect dump (${OUTSUB}) ==="
