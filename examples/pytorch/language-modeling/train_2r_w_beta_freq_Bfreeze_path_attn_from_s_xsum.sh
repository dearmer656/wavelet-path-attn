#! /bin/bash
#SBATCH --job-name=
#SBATCH --output=log_file/train/%j_2r_w_beta_freq_Bfreeze_path_attn_from_s_xsum.txt
...
--eval_strategy steps --eval_steps 250 --logging_dir ./2r_w_beta_freq_Bfreeze_path_attn_from_s_xsum_log --logging_steps 250 --save_steps 250 --attn_implementation path_attn_wfreq --path_use_qk_norm false \
--path_use_low_rank_w true --path_use_w_shortconv false --path_conv_size 3 --warmup_ratio 0.05 --path_conv_bias false --output_dir runs/2r_w_beta_freq_Bfreeze_path_attn_from_s_xsum \
--gradient_accumulation_steps 2 --b_unfreeze_step 5000 --pe_method vanilla --single_A_B False \
--use_beta_modulation True --model_name_or_path runs/2r_w_beta_freq_Bfreeze_path_attn_from_s/checkpoint-80000
