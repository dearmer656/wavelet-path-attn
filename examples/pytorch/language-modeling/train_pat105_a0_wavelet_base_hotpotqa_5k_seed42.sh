#! /bin/bash
#SBATCH --job-name=PAT105_A0
#SBATCH --output=log_file/train/%j_pat105_a0_wavelet_base.txt
...
# PAT-105: A0 — wavelet baseline (no path blend), matched-budget comparison for PAT-105.
# Starting checkpoint: gpt2_medium_0.01wd_0.1wu_pytorch_level_path_attn/checkpoint-30000
# Dataset: hotpot_qa distractor, max_steps=5000, global_bs=8*2*4=64.
# Frozen recipe for PAT-102/105 baseline comparison.
...
MASTER_PORT=$(( 20100 + SLURM_JOB_ID % 1000 ))
...
/cl/work5/hongyu-s/conda/envs/latest_transformers/bin/torchrun \
  ...
  --output_dir runs/pat105_a0_wavelet_base_hotpotqa_5k_seed42 \
  --logging_dir ./pat105_a0_wavelet_base_hotpotqa_5k_seed42_log \
  --cfg_path runs/pat105_a0_wavelet_base_hotpotqa_5k_seed42/supply_model.cfg

echo "=== PAT-105 A0 wavelet baseline done ==="
