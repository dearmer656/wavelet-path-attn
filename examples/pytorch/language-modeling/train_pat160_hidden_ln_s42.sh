#! /bin/bash
#SBATCH --job-name=PAT160_hln_s42
#SBATCH --output=log_file/train/%j_pat160_hidden_ln_s42.txt
...
# instead of PaTH q_corr delta. One-factor change from Full QWAB (s42):
#   wavelet_ctx_feat_mode="hidden_ln" (was "q_minus_qcorr_meanh")
# All other flags, backbone, data, LR, epochs identical to Full QWAB s42.
...
echo '================= BEGIN RUN PAT-160 hidden_ln s42 ================='
...
/cl/work5/hongyu-s/conda/envs/latest_transformers/bin/torchrun \
  ...
  --output_dir runs/mix_medium_owt_hidden_ln_10ep_s42 \
  --logging_dir ./log_file/mix_medium_owt_hidden_ln_10ep_s42 \
  --cfg_path runs/mix_medium_owt_hidden_ln_10ep_s42/supply_model.cfg

echo "=== PAT-160 QWAB-Hidden-LN s42 done ==="
