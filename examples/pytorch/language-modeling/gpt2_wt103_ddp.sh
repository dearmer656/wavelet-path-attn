#! /bin/bash
#SBATCH --job-name=
#SBATCH --output=log_file/test/%j_gpt2_wt103_ddp_step5000_test.txt
...
#################################################### for alibi #################################################### for alibi ###############################################                
torchrun --nproc_per_node=1 ./run_clm.py --model_type gpt2 --tokenizer_name gpt2 --model_name_or_path "./runs/gpt2_wt103_ddp/checkpoint-5000" --share_freq_across_heads False \
--learning_rate 1e-4 --per_device_train_batch_size 1  --per_device_eval_batch_size 1 --block_size 512 --dataset_name wikitext \
--dataset_config_name wikitext-103-raw-v1 --do_eval --overwrite_output_dir --num_train_epochs 30 --output_dir "./gpt2_wt103_ddp" --num_harmonics 1 \
--eval_strategy steps --logging_dir ./gpt2_wt103_ddp_log --logging_steps 250 --save_steps 2500 --attn_implementation path_attn --path_use_qk_norm false \
--path_use_low_rank_w true --path_use_w_shortconv false --path_conv_size 3 --warmup_steps 5000 --path_conv_bias false --output_dir runs/gpt2_wt103_ddp \
