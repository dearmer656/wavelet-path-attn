#! /bin/bash
#SBATCH --job-name=OWT_MedQWAB_HP
#SBATCH --output=log_file/train/%j_owt_medium_QWAB_hotpot.txt
...
# PAT-101: QWAB downstream fine-tuning on HotpotQA-Long, from OWT-pretrained medium backbone.
...
/project/nlp-work5/hongyu-s/conda/envs/latest_transformers/bin/torchrun --nproc_per_node=2 --master_port=12833 ./run_clm.py \
  ...
  --output_dir runs/owt_medium_QWAB_hotpot \
  --logging_dir ./owt_medium_QWAB_hotpot_log \
  --cfg_path runs/owt_medium_QWAB_hotpot/supply_model.cfg
