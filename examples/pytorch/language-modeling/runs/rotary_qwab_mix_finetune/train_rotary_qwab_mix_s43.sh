...
# PAT-164: Rotary + QWAB mix fine-tune seed=43
# Baseline: standard Rotary attention (train_rotary_mix_s43.sh)
...
if [ -n "${SLURM_NODELIST:-}" ]; then
  ...
  export MASTER_PORT=$(( 19400 + SLURM_JOB_ID % 1000 ))
...
RUN_OUT="${WORKDIR}/runs/rotary_qwab_mix_finetune/s43"
mkdir -p "${RUN_OUT}/train"

MASTER_PORT=$(( 19400 + SLURM_JOB_ID % 1000 ))

_JOB_TAG="rot_qwab_s43_${SLURM_JOB_ID:-local}"
...
echo "=== Done: Rotary+QWAB mix fine-tune s43 ==="
echo "Output: ${RUN_OUT}"
