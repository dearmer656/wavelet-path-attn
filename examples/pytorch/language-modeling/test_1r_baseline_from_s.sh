#! /bin/bash
#SBATCH --job-name=
#SBATCH --output=log_file/test/%j_1r_baseline_from_s_step80000_test.txt
...
#################################################### for alibi #################################################### for alibi ###############################################                
torchrun --nproc_per_node=2 --master_port=12425 ./run_clm.py --model_type gpt2 --tokenizer_name gpt2 --model_name_or_path "./runs/1r_baseline_from_s/checkpoint-80000" --share_freq_across_heads True \
--learning_rate 1e-4 --per_device_train_batch_size 1  --per_device_eval_batch_size 1 --block_size 512 --dataset_name wikitext \
--dataset_config_name wikitext-103-raw-v1 --do_eval --overwrite_output_dir --num_train_epochs 30 --output_dir "./1r_baseline_from_s" \
--eval_strategy steps --logging_dir ./1r_baseline_from_s_log --logging_steps 250 --save_steps 2500 --attn_implementation path_attn --path_use_qk_norm false \
--path_use_low_rank_w true --path_use_w_shortconv false --path_conv_size 3 --warmup_steps 5000 --path_conv_bias false --output_dir runs/1r_baseline_from_s \
--gradient_accumulation_steps 2 --num_harmonics 1 --single_A_B True \
