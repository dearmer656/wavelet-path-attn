#!/bin/bash
#SBATCH --job-name=hp_rot_qwab
#SBATCH --output=/cl/work5/hongyu-s/transformers/examples/pytorch/language-modeling/runs/small_rotary_qwab_10ep_s42/ckpt_eval_hm/%j_hotpot_rotary_qwab_s42.txt
...
# PAT-166: HotpotQA-Long eval for Rotary+QWAB small model, seed=42
...
echo "=== Rotary+QWAB HotpotQA-Long eval | ckpt: ${BEST_CKPT} ==="
...
for BLOCK_SIZE in 512 2048 4096; do
  ...
  mkdir -p "${OUT_DIR}"
  MASTER_PORT=$(( 13000 + JOB_PORT_OFFSET ))
...
echo "=== Done: Rotary+QWAB HotpotQA eval. Results: ${RESULT_DIR} ==="
